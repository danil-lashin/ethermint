...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		default:
			return nil, errors.New("unknown query endpoint")
		}
	}
}
...
func queryBlockLogsBloom(ctx sdk.Context, path []string, keeper Keeper) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("could not unmarshall block number: %s", err.Error()))
	}

	bloom, err := keeper.GetBlockBloomMapping(ctx, num)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("failed to get block bloom mapping: %s", err.Error()))
	}
